...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    if (!LoadFromObjectFile("CD.obj", &vertices, &indices, &vertsCnt, &indsCnt)) exit(-1);
    ...
    while (msg.message != WM_QUIT)
    {
        if (PeekMessage(&msg, 0, 0, 0, PM_REMOVE)) { TranslateMessage(&msg); DispatchMessage(&msg); }
            ...
            XMMATRIX model = XMMatrixScaling(0.6f, 0.6f, 0.6f) * XMMatrixRotationY(t / 2.0f) * XMMatrixRotationX(t / 3.0f);
            XMMATRIX view = XMMatrixLookAtLH({ 0, 1, -10 }, { 0, 0, 0 }, { 0, 1, 0 });
